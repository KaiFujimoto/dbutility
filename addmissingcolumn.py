...
def add_missing(cursorremote, cursorlocal, local_missing, table_name, table_schema):
    print(table_name)
    print(table_schema)
    for missing in local_missing:
        print("Adding this:", missing)
        previous_column = missing[0]
        column_name = missing[1]
        #grabbing the schema information of the missing table from remote
        remoteschema = f"select column_name, column_default, is_nullable, column_type, column_key, extra, collation_name from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = '{table_name}' and TABLE_SCHEMA = '{table_schema}' and COLUMN_NAME='{column_name}'"
        ...
        # print("\n")
        executecreate = f"alter table {table_name} add {createdcolumns} {previous_column}"
        print("\n")
        print(executecreate)
